...
local lume = require "scripts/lib/lume"
...
local lastmodified = function(f) return love.filesystem.getInfo(f).modtime end
  ...
  for _, f in pairs(lume.map(dir(path), fullpath)) do
    if not skipdotfiles or not f:match("/%.[^/]*$") then
      if recursive and isdir(f) then
        -- NOTE: lume.merge() does't work.
        -- lume.merge(t, lurker.listdir(f, recursive, skipdotfiles))
        for _, v in pairs(lurker.listdir(f, recursive, skipdotfiles)) do
          table.insert(t, v)
        end
      else
...
function lurker.getchanged()
  local function fn(f)
    -- if f:match('%.lua$') then
    --   print(f, ' old:' .. tostring(lurker.files[f]), ' new:' .. tostring(lastmodified(f)))
    -- end
    return f:match("%.lua$") and lurker.files[f] ~= lastmodified(f)
  end
  -- print('===================lurker.getchanged()')
  -- print(inspect.inspect(lurker.listdir(lurker.path, true, true)))
...
function lurker.restartlove()
  print 'restarting love'
  love.run()
end
